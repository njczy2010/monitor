...
class UnitTestCase extends SimpleTestCase
{
	...
	//add by czy 20160720 for monitor
	public function execUnknown($url,$path = false,$para = false,$headers = false,$timeout = 3000) {
         $handler = curl_init();
         $this->assertTrue($handler);
		 curl_setopt($handler, CURLOPT_CONNECTTIMEOUT_MS, $timeout);
         curl_setopt($handler, CURLOPT_RETURNTRANSFER, true);
		 //$header_total = array('Host: 2ab862678.yunlian.io');
		 $header_total = array("Host: " . $GLOBALS["G_CONFIG"]["yunlian"]["domain"]);
		 if($headers){
			 $header_total = array_merge($headers,$header_total);
		 }
		 if($path) {
			 if (@$_GET['nginx_ip']) {
				curl_setopt($handler, CURLOPT_HTTPHEADER, $header_total);
	...
	//add by czy 20160720 for monitor
	public function execSuccess($url,$path = false,$para = false,$headers = false,$timeout = 3000) {
         $handler = curl_init();
         //$this->assertTrue($handler);
		 curl_setopt($handler, CURLOPT_CONNECTTIMEOUT_MS, $timeout);
         curl_setopt($handler, CURLOPT_RETURNTRANSFER, true);
	...
	//add by czy 20160720 for monitor
	public function execFail($url,$path = false,$para = false,$headers = false,$timeout = 3000) {
         $handler = curl_init();
         //$this->assertTrue($handler);
         curl_setopt($handler, CURLOPT_CONNECTTIMEOUT_MS, $timeout);
         curl_setopt($handler, CURLOPT_RETURNTRANSFER, true);
         ...
         $result = curl_exec($handler);
         $retInfo = curl_getinfo($handler);
         //$this->assertNotEqual($retInfo["http_code"], 200);
         curl_close($handler);
